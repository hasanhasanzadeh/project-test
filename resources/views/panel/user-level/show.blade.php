    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs border" >
            <table class="w-full  order">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">{{__('dashboard.level_name')}}</td>
                ...
                <tr class="text-gray-700 dark:text-gray-400 mx-auto">
                    ...
                    <td class="px-4 py-3 text-xs">
                        <img src="{{$userLevel->photo?$userLevel->photo->address:''}}" height="100" class="object-cover rounded shadow mx-auto" width="100" alt="">
                    </td>
                </tr>
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">{{__('dashboard.level_number')}}</td>
                    <td class="px-4 py-3 text-xs">
                        {{$userLevel->level_number}}
                    </td>
                </tr>
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">{{__('dashboard.wage')}}</td>
                    <td class="px-4 py-3 text-sm">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">{{__('dashboard.maximum_deposit')}}</td>
                    <td class="px-4 py-3 text-sm">
                       {{ number_format($userLevel->maximum_deposit,0) }}
                    </td>
                </tr>
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">{{__('dashboard.maximum_withdrawal')}}</td>
                    <td class="px-4 py-3 text-sm">
                       {{ number_format($userLevel->maximum_withdrawal,0) }}
                    </td>
                </tr>
                <tr class="text-gray-700 dark:text-gray-400">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm">
                        @if(config('app.locale')=='fa')
                            {{verta()->instance($userLevel->created_at)->format('%d %B %Y')}}
                        @else
                            {{ date('d-M-y', strtotime($userLevel->created_at))}}
                        @endif
                    </td>
                </tr>
                </tbody>
