...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('verify-samples.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs border" >
                        ...
                        @foreach($verifySamples as $verifySample)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-xs">
                                    <img src="{{$verifySample->photo?$verifySample->photo->address:asset('images/no-image.png')}}" height="100" class="object-cover rounded shadow mx-auto" width="100" alt="">
                                </td>
                                <td class="px-4 py-3 text-xs">
                                    {{$verifySample->title}}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @if($verifySample->status)
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if(config('app.locale')=='fa')
                                        {{verta()->instance($verifySample->created_at)->format('%d %B %Y')}}
                                    @else
                                        {{ date('d-M-y', strtotime($verifySample->created_at))}}
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class="text-xl flex justify-center items-center">
                                    <a href="{{route('verify-samples.show',$verifySample->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('verify-samples.edit',$verifySample->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                        <form action="{{route('verify-samples.destroy',$verifySample->id)}}" method="POST" class="mx-auto">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$verifySamples->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$verifySamples->count()}}
                </span>
                  ...
                  <nav aria-label="Table navigation">
                    <ul class="inline-flex items-center px-4" dir="ltr">
                        {!! $verifySamples->appends(Request::except('page'))->render() !!}
                    </ul>
                  </nav>
                </span>
                </div>
