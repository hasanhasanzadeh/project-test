                ...
                <form class="w-full" method="post" action="{{route('pages.update',$page->id)}}" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-wrap mx-3 mb-6">
                        <div class="w-full md:w-1/2 px-3 my-3">
                            ...
                            <input class="appearance-none block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="title" name="title" type="text" value="{{$page->title}}" placeholder="{{__('dashboard.enterTitle')}}">
                        </div>
                        <div class="w-full md:w-1/2 px-3 my-3">
                            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2 dark:text-gray-50" for="slug">
                                {{__('dashboard.slug')}}
                            </label>
                            <input class="appearance-none block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="slug" name="slug" type="text" value="{{$page->slug}}" placeholder="{{__('dashboard.enterSlug')}}">
                        </div>
                        ...
                        <div class="w-full px-3  my-3">
                            ...
                            <textarea name="description" class="form-textarea appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder="{{__('dashboard.enterDescription')}}" id="description" cols="30" rows="10">{!! $page->description!!}</textarea>
                        </div>
                        <div class="w-full px-3 my-3">
                            ...
                            <img id="image-select" @if($page->photo) src="{{$page->photo->address}}" @endif alt="" class="object-cover shadow rounded @if(!$page->photo) hidden @endif" width="150" height="150">
                        </div>
