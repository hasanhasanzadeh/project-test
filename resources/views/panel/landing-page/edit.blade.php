    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs border" >
            <div class="w-full overflow-x-auto">
                <form class="w-full" method="post" action="{{route('landing-pages.update',$landingPage->id)}}" enctype="multipart/form-data">
                    @csrf
                    {{method_field('PATCH')}}
                    ...
                    <div class="flex flex-wrap mx-3 mb-6">
                        <div class="w-full md:w-1/2 px-3 my-3">
                            ...
                            <input class="appearance-none block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="title" name="title" type="text" value="{{$landingPage->title}}" placeholder="{{__('dashboard.enterTitle')}}">
                        </div>
                        <div class="w-full md:w-1/2 px-3 my-3">
                            ...
                            <input class="appearance-none block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="landing_number" name="landing_number" type="number" value="{{$landingPage->landing_number}}" placeholder="{{__('dashboard.landing_number')}}">
                        </div>
                        <div class="w-full md:w-1/2 px-3  my-3">
                            ...
                            <select name="status" id="status" class="form-select appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-14 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                <option value="1" @if($landingPage->status==1) selected @endif>{{__('dashboard.active')}}</option>
                                <option value="0" @if($landingPage->status==0) selected @endif>{{__('dashboard.inactive')}}</option>
                            </select>
                        </div>
                        <div class="w-full  md:w-1/2 px-3  my-3">
                            ...
                            <select name="type" id="type" class="form-select appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-14 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                <option value="top" @if($landingPage->type=='top') selected @endif>بالا</option>
                                <option value="bottom" @if($landingPage->type=='bottom') selected @endif>پایین</option>
                                <option value="base" @if($landingPage->type=='base') selected @endif>اصل و اساس</option>
                                <option value="attribute" @if($landingPage->type=='attribute') selected @endif>ویژگی</option>
                                <option value="introduction" @if($landingPage->type=='introduction') selected @endif>معرفی</option>
                            </select>
                        </div>
                        <div class="w-full px-3  my-3">
                            ...
                            <textarea name="description" class="form-textarea appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder="{{__('dashboard.enterDescription')}}" id="description" cols="30" rows="10">{!! $landingPage->description!!}</textarea>
                        </div>
                        <div class="w-full px-3 my-3">
                            ...
                            <img id="image-select" @if($landingPage->photo) src="{{$landingPage->photo->address}}" @endif alt="" class="object-cover shadow rounded @if(!$landingPage->photo) hidden @endif" width="150" height="150">
                        </div>
                        <div class="w-full px-3  my-3">
